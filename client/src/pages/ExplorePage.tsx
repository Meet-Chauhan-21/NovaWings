...
const CITY_GRADIENTS = [
  "linear-gradient(145deg,var(--nw-bg) 0%,var(--nw-accent-blue) 100%)",
  "linear-gradient(145deg,var(--nw-warm-bg) 0%,var(--nw-primary-dark) 100%)",
  "linear-gradient(145deg,var(--nw-bg) 0%,var(--nw-success) 100%)",
  "linear-gradient(145deg,var(--nw-elevated) 0%,var(--nw-accent-violet) 100%)",
  "linear-gradient(145deg,var(--nw-warm-bg) 0%,var(--nw-secondary) 100%)",
  "linear-gradient(145deg,var(--nw-bg) 0%,var(--nw-accent-teal) 100%)",
  "linear-gradient(145deg,var(--nw-elevated) 0%,var(--nw-accent-pink) 100%)",
  "linear-gradient(145deg,var(--nw-bg) 0%,var(--nw-info) 100%)",
];
...
export default function ExplorePage() {
  ...
  // ────────────────────────────────────────────────────────
  return (
    <Box sx={{ minHeight: "100vh", background: "var(--nw-bg)" }}>
      ...
      >
        <Box sx={{ position: "absolute", top: "60%", left: "50%", transform: "translate(-50%,-50%)", width: 600, height: 300, borderRadius: "50%", background: "radial-gradient(ellipse,var(--nw-primary-12) 0%,transparent 70%)", pointerEvents: "none" }} />

        <Box sx={{ maxWidth: 720, mx: "auto", px: 3, textAlign: "center", position: "relative", zIndex: 1 }}>
          <Box sx={{ display: "inline-flex", alignItems: "center", gap: 1, background: "var(--nw-primary-10)", border: "1px solid rgba(249,115,22,0.25)", borderRadius: "100px", px: 2, py: 0.75, mb: 3 }}>
            <FlightTakeoffIcon sx={{ fontSize: 14, color: "var(--nw-primary)" }} />
            <Typography sx={{ color: "var(--nw-primary)", fontSize: "0.75rem", fontWeight: 600, letterSpacing: "0.06em", textTransform: "uppercase" }}>
              Explore Mode
            </Typography>
          </Box>
          <Typography variant="h3" sx={{ color: "var(--nw-text-primary)", fontWeight: 800, mb: 1.5, fontSize: { xs: "1.875rem", md: "2.5rem" } }}>
            Explore{" "}
            <Box component="span" sx={{ background: "linear-gradient(90deg,var(--nw-primary),var(--nw-secondary))", WebkitBackgroundClip: "text", WebkitTextFillColor: "transparent" }}>
              Destinations
            </Box>
          </Typography>
          <Typography sx={{ color: "var(--nw-text-secondary)", fontSize: "1.0625rem", lineHeight: 1.7 }}>
      ...
      <Box sx={{ maxWidth: 1100, mx: "auto", px: { xs: 2, md: 3 }, mt: -5, position: "relative", zIndex: 10 }}>
        <Box sx={{ background: "var(--nw-card)", border: "1px solid var(--nw-border-strong)", borderRadius: "20px", p: { xs: 2.5, sm: 3 }, boxShadow: "0 16px 48px rgba(0,0,0,0.4)" }}>
          <Box sx={{ display: "flex", flexWrap: "wrap", gap: 2, alignItems: "center" }}>
            <Box sx={{ flex: "1 1 180px", minWidth: 160 }}>
              <Typography sx={{ color: "var(--nw-text-muted)", fontSize: "0.6875rem", fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.08em", mb: 0.75 }}>From</Typography>
            ...
            <Box sx={{ flex: "1 1 180px", minWidth: 160 }}>
              <Typography sx={{ color: "var(--nw-text-muted)", fontSize: "0.6875rem", fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.08em", mb: 0.75 }}>To</Typography>
              <CityCombobox value={toCity} onChange={setToCity} placeholder="Destination city" excludeCity={fromCity} type="arrival" />
            </Box>
            <Box sx={{ minWidth: 150 }}>
              <Typography sx={{ color: "var(--nw-text-muted)", fontSize: "0.6875rem", fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.08em", mb: 0.75 }}>Date</Typography>
              <DateInput value={dateFilter} onChange={setDateFilter} min={today} showQuickButtons />
            </Box>
            <Button onClick={handleFind} variant="contained" startIcon={<SearchIcon />} sx={{ px: 3, py: 1.4, borderRadius: "12px", fontWeight: 700, background: "linear-gradient(135deg,var(--nw-primary),var(--nw-primary-dark))", boxShadow: "0 4px 16px rgba(249,115,22,0.35)", "&:hover": { boxShadow: "0 6px 22px rgba(249,115,22,0.5)" }, whiteSpace: "nowrap" }}>
              Find Flights
            </Button>
            {fromCity && toCity && (
              <Button
                ...
                sx={{ px: 2.5, py: 1.4, borderRadius: "12px", fontWeight: 600, borderColor: "rgba(249,115,22,0.35)", color: "var(--nw-primary)", "&:hover": { borderColor: "var(--nw-primary)", background: "var(--nw-primary-06)" }, whiteSpace: "nowrap" }}
              >
                Full Search
              </Button>
            )}
      ...
      <Box sx={{ maxWidth: 1280, mx: "auto", px: { xs: 2, md: 3 }, py: 5 }}>
        ...
        {/* Breadcrumb */}
        <Breadcrumbs separator={<NavigateNextIcon sx={{ fontSize: 14, color: "var(--nw-text-disabled)" }} />} sx={{ mb: 4 }}>
          <Typography
            onClick={clearFrom}
            sx={{ color: !fromCity ? "var(--nw-text-primary)" : "var(--nw-primary)", fontWeight: fromCity ? 600 : 500, cursor: fromCity ? "pointer" : "default", fontSize: "0.875rem", "&:hover": fromCity ? { textDecoration: "underline" } : {} }}
          >
            Explore
          </Typography>
          {fromCity && (
            <Box sx={{ display: "flex", alignItems: "center", gap: 0.75 }}>
              <Typography sx={{ color: "var(--nw-primary)", fontWeight: 600, fontSize: "0.875rem" }}>{fromCity}</Typography>
              <Box component="button" onClick={clearFrom} sx={{ border: "none", background: "none", cursor: "pointer", color: "var(--nw-text-disabled)", "&:hover": { color: "var(--nw-error)" }, display: "flex", p: 0 }}>
          ...
          {toCity && (
            <Box sx={{ display: "flex", alignItems: "center", gap: 0.75 }}>
              <Typography sx={{ color: "var(--nw-primary)", fontWeight: 600, fontSize: "0.875rem" }}>{toCity}</Typography>
              <Box component="button" onClick={clearTo} sx={{ border: "none", background: "none", cursor: "pointer", color: "var(--nw-text-disabled)", "&:hover": { color: "var(--nw-error)" }, display: "flex", p: 0 }}>
                <CloseIcon sx={{ fontSize: 14 }} />
              </Box>
            </Box>
          )}
        ...
        <AnimatePresence mode="wait">
          ...
          {pageState === "CITY_SELECT" && (
            <motion.div key="city-select" initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -8 }} transition={{ duration: 0.3 }}>
              <Typography variant="h5" sx={{ color: "var(--nw-text-primary)", fontWeight: 700, mb: 1 }}>Where are you flying from?</Typography>
              <Typography sx={{ color: "var(--nw-text-muted)", fontSize: "0.9375rem", mb: 4 }}>Pick a city to see all available routes</Typography>
                    ...
                    <motion.div key={loc.id} variants={gridItem}>
                      <Box
                        ...
                        sx={{
                          ...
                          "&:hover": { border: "1px solid rgba(249,115,22,0.5)", boxShadow: "0 8px 28px var(--nw-primary-15)", transform: "translateY(-3px) scale(1.01)" },
                          "&:hover .city-overlay": { opacity: 1 },
                        }}
                      >
                        {/* Airport code watermark */}
                        <Typography sx={{ position: "absolute", bottom: -8, right: 8, fontSize: "4rem", fontWeight: 900, fontFamily: '"JetBrains Mono",monospace', color: "var(--nw-border)", lineHeight: 1, userSelect: "none", letterSpacing: "-0.04em" }}>
                          {loc.airportCode}
                        </Typography>
                        {/* Hover overlay */}
                        <Box className="city-overlay" sx={{ position: "absolute", inset: 0, background: "var(--nw-primary-06)", opacity: 0, transition: "opacity 0.25s ease" }} />
                        {/* Routes badge */}
                        <Box sx={{ position: "absolute", top: 10, right: 10 }}>
                          <Chip label={`${loc.activeFlights ?? 0} routes`} size="small" sx={{ background: "rgba(0,0,0,0.45)", color: "rgba(255,255,255,0.92)", fontSize: "0.6875rem", fontWeight: 600, height: 20, "& .MuiChip-label": { px: 1 } }} />
                        </Box>
                        {/* City info */}
                        <Box sx={{ position: "absolute", bottom: 0, left: 0, right: 0, p: 1.75, background: "linear-gradient(to top,rgba(0,0,0,0.75) 0%,transparent 100%)" }}>
                          ...
                          <Typography sx={{ color: "rgba(255,255,255,0.72)", fontSize: "0.6875rem" }}>{loc.state}</Typography>
                        </Box>
                      </Box>
                    </motion.div>
                  ))}
          ...
          {pageState === "ROUTE_SELECT" && (
            <motion.div key="route-select" initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -8 }} transition={{ duration: 0.3 }}>
              <Typography variant="h5" sx={{ color: "var(--nw-text-primary)", fontWeight: 700, mb: 1 }}>
                Flights from{" "}
                <Box component="span" sx={{ background: "linear-gradient(90deg,var(--nw-primary),var(--nw-secondary))", WebkitBackgroundClip: "text", WebkitTextFillColor: "transparent" }}>{fromCity}</Box>{" "}
                to…
              </Typography>
              <Typography sx={{ color: "var(--nw-text-muted)", fontSize: "0.9375rem", mb: 4 }}>Choose a destination to see available flights</Typography>
                ...
                <motion.div variants={gridContainer} initial="hidden" animate="visible" style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill,minmax(280px,1fr))", gap: "16px" }}>
                    ...
                    return (
                      <motion.div key={`${route.destination}-${idx}`} variants={gridItem}>
                        <Box
                          ...
                          sx={{ background: "var(--nw-card)", border: "1px solid var(--nw-border)", borderRadius: "16px", p: 2.5, cursor: "pointer", transition: "all 0.25s ease", "&:hover": { border: "1px solid rgba(249,115,22,0.35)", boxShadow: "0 8px 28px var(--nw-primary-10)", transform: "translateY(-2px)" } }}
                        >
                          <Box sx={{ display: "flex", alignItems: "center", gap: 1, mb: 2 }}>
                            <Typography sx={{ color: "var(--nw-text-primary)", fontWeight: 700, fontSize: "1.0625rem" }}>{fromCity}</Typography>
                            <FlightTakeoffIcon sx={{ color: "var(--nw-primary)", fontSize: 16, mx: 0.25 }} />
                            <Typography sx={{ color: "var(--nw-text-primary)", fontWeight: 700, fontSize: "1.0625rem" }}>{route.destination}</Typography>
                            <Box sx={{ ml: "auto" }}><StarIcon sx={{ fontSize: 14, color: "var(--nw-primary-40)" }} /></Box>
                          </Box>
                          <Typography sx={{ color: "var(--nw-text-muted)", fontSize: "0.8125rem", mb: 1.5 }}>{route.airline}</Typography>
                          <Box sx={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                            <Box>
                              <Typography sx={{ color: "var(--nw-text-disabled)", fontSize: "0.6875rem" }}>from</Typography>
                              <Typography sx={{ color: "var(--nw-primary)", fontWeight: 800, fontSize: "1.25rem", lineHeight: 1 }}>
                                ₹{route.cheapestPrice.toLocaleString("en-IN")}
                              </Typography>
                            </Box>
                            <Box sx={{ display: "flex", flexDirection: "column", alignItems: "flex-end", gap: 0.5 }}>
                              <Chip label={`${route.flightCount} flights`} size="small" sx={{ background: "var(--nw-primary-10)", color: "var(--nw-primary)", border: "1px solid var(--nw-primary-20)", fontSize: "0.6875rem", fontWeight: 600, height: 20, "& .MuiChip-label": { px: 1 } }} />
                              <Button
                                size="small"
                                ...
                                sx={{ color: "var(--nw-primary)", fontSize: "0.75rem", fontWeight: 600, p: 0, minWidth: 0, "&:hover": { background: "none", textDecoration: "underline" } }}
                              >
                                View all →
                              </Button>
                            </Box>
                  ...
                  {routeQueries.every((q) => !q.isLoading && !q.data) && (
                    <Box sx={{ gridColumn: "1/-1", textAlign: "center", py: 8 }}>
                      <Typography sx={{ color: "var(--nw-text-disabled)", fontSize: "1rem" }}>No flights found from {fromCity}.</Typography>
                    </Box>
                  )}
                </motion.div>
              )}
          ...
          {pageState === "FLIGHT_LIST" && (
            <motion.div key="flight-list" initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -8 }} transition={{ duration: 0.3 }}>
              ...
              <Box sx={{ display: "flex", flexWrap: "wrap", alignItems: "flex-start", justifyContent: "space-between", gap: 2, mb: 3 }}>
                <Box>
                  <Box sx={{ display: "flex", alignItems: "center", gap: 1.5, mb: 0.5 }}>
                    <Typography variant="h5" sx={{ color: "var(--nw-text-primary)", fontWeight: 800 }}>{fromCity}</Typography>
                    <FlightTakeoffIcon sx={{ color: "var(--nw-primary)", fontSize: 22 }} />
                    <Typography variant="h5" sx={{ color: "var(--nw-text-primary)", fontWeight: 800 }}>{toCity}</Typography>
                  </Box>
                ...
                <Button
                  ...
                  variant="outlined"
                  size="small"
                  sx={{ borderRadius: "8px", borderColor: "var(--nw-border-strong)", color: "var(--nw-text-secondary)", "&:hover": { borderColor: "var(--nw-primary-30)", color: "var(--nw-primary)", background: "var(--nw-primary-04)" } }}
                >
                  ← Change Destination
                </Button>
              </Box>
              ...
              <ToggleButtonGroup
                ...
                sx={{ mb: 3, "& .MuiToggleButton-root": { border: "1px solid var(--nw-border-strong)", color: "var(--nw-text-secondary)", borderRadius: "100px !important", px: 2.5, py: 0.75, fontSize: "0.8125rem", fontWeight: 600, mx: 0.5, textTransform: "none", transition: "all 0.2s ease", "&.Mui-selected": { background: "var(--nw-primary-15)", color: "var(--nw-primary)", borderColor: "var(--nw-primary-40)" }, "&:hover": { background: "var(--nw-border-soft)" } } }}
              >
              ...
              {flightsLoading ? (
                ...
                <Box sx={{ textAlign: "center", py: 10 }}>
                  <Typography sx={{ color: "var(--nw-text-disabled)", fontSize: "1rem" }}>
                ...
                <motion.div variants={gridContainer} initial="hidden" animate="visible" style={{ display: "flex", flexDirection: "column", gap: "12px", maxWidth: 860 }}>
                  ...
                  {visibleCount < sortedFlights.length && (
                    <Button
                      ...
                      variant="outlined"
                      sx={{ alignSelf: "center", mt: 1, borderRadius: "10px", borderColor: "var(--nw-border-strong)", color: "var(--nw-text-secondary)", px: 4, "&:hover": { borderColor: "var(--nw-primary-30)", color: "var(--nw-primary)", background: "var(--nw-primary-04)" } }}
                    >
